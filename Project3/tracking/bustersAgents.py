...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        bestAction = None
        closestDistAfterAction = 88888

        for action in legal:
            posAfterAction = Actions.getSuccessor(pacmanPosition,action)
            distance = self.distancer.getDistance(closestPos, posAfterAction)
            if distance < closestDistAfterAction:
                closestDistAfterAction = distance
                bestAction = action

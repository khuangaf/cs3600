...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        # print "++++++++++++++++++++++++++++++++++++++++++"
        # print self.weights
        # print len(inActs), len(self.weights)
        for i, act in enumerate(inActs):
            modification = alpha*act*delta
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for h in range(self.numHiddenLayers):
            newInActs= []
            for p in range(self.layerSize[h+1]):
                newInActs.append(self.hiddenLayers[h][p].sigmoidActivation(inActs))
            # print output
            inActs = newInActs
            output.append(inActs)
        newInActs = []
        for i in range(self.layerSize[-1]):
            newInActs.append(self.outputLayer[i].sigmoidActivation(inActs))
        output.append(newInActs)
        return output
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            for outputNum in xrange(len(example[1])):
                ...
                # print len(self.outputLayer[outputNum].weights)
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    # print len(layer[neuronNum].weights), len(allLayerOutput[layerNum])
                    ...
                    for i in range(len(nextLayer)):
                        # print len(nextDelta), len(nextLayer)
                        weightDelta += nextDelta[i]*nextLayer[i].weights[neuronNum+1]

                    delta = gPrime*weightDelta
            ...
            for numLayer in xrange(0,self.numLayers):
                ...
                for numNeuron in xrange(len(layer)):
                    ...
                    # print len(inActs), len(deltas[numLayer])
                    # print alpha
                    # print len( layer[numNeuron].weights)
                    weightMod = layer[numNeuron].updateWeights(inActs = inActs, alpha = alpha, delta = deltas[numLayer][numNeuron])
